...
export class HomestayUserComponent implements OnInit {
  ...
  searchHomeStay: HomeStayResponse[];
  ...
  searchPlace($event: any) {
    this.listHomeStay = this.searchHomeStay;
    const idPlace = $event;
    if (idPlace === '') {
      this.listHomeStay = this.searchHomeStay;
    } else {
      this.listHomeStay = this.listHomeStay.filter(
        (option) =>
          option.detailPlace.find(value => value.id_place === idPlace),
      );
    }
  }
  ...
  searchCity() {
    this.listHomeStay = this.searchHomeStay;
    const cityValue = this.homeStayForm.get('cityName')?.value.toLowerCase();
    if (cityValue !== '') {
      this.listHomeStay = this.listHomeStay.filter(
        (option) =>
          option.cityName.toLowerCase() === cityValue,
      );
    } else {
      this.listHomeStay = this.searchHomeStay;
    }
  ...
  searchDistrict() {
    this.listHomeStay = this.searchHomeStay;
    const districtValue = this.homeStayForm.get('districtName')?.value.toLowerCase();
    if (districtValue !== '') {
      this.listHomeStay = this.listHomeStay.filter(
        (option) =>
          option.districtName.toLowerCase() === districtValue,
      );
    } else {
     this.searchCity();
    }
  ...
  searchVillage() {
    this.listHomeStay = this.searchHomeStay;
    const villageValue = this.homeStayForm.get('villageName')?.value.toLowerCase();
    if (villageValue !== '') {
      this.listHomeStay = this.listHomeStay.filter(
        (option) =>
          option.villageName.toLowerCase() === villageValue,
      );
    } else {
      this.searchDistrict();
    }
