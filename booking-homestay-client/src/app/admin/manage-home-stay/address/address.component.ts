    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === true) {
        this.addressService.deleteCity(id).subscribe(
          ...
          (error) => {
            this.toastrService.showToast('danger', 'Thất bại', 'Thành phố đã tồn tại quận');
            throwError(error);
          },
        );
      }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === true) {
        this.addressService.deleteDistrict(id).subscribe(
          ...
          (error) => {
            throwError(error);
            this.toastrService.showToast('danger', 'Thất bại', 'Quận đã tồn tại phường');
          },
        );
      }
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === true) {
        this.addressService.deleteVillage(id).subscribe(
          ...
          (error) => {
            throwError(error);
            this.toastrService.showToast('danger', 'Thất bại', 'Phường đã tồn tại cơ sở');
          },
        );
      }
    });
